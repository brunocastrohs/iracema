...
export default function Chat() {
    ...
    // NOVO: modo e tabela selecionada
    ...
    const [messages, setMessages] = useState([
        {
            ...
            ts: now(),
            text:
                "Me diga o que dizer no catálogo (tema, recorte, ano, fonte). " +
    ...
    async function handleAskFlow(question) {
        ...
        // remove o "Consultando…" (última msg assistant) e substitui
        setMessages((prev) => {
            const copy = [...prev];
            // remove última se for exatamente "Consultando…"
            const last = copy[copy.length - 1];
            if (last?.role === "assistant" && last?.text === "Consultando…") {
                copy.pop();
            }
            if (!ok) {
                copy.push({ role: "assistant", ts: now(), text: `Erro: ${error || "falha no /chat/ask"}` });
            } else {
                if (data?.error) {
    ...
    function onSend(textFromChip) {
        ...
        if (mode === "ask") {
            // se o usuário clicou “Nova pergunta”, só pede a pergunta
        ...
        const suggestions = top.slice(0, 5).map((r) => ({
            id: r.id,
            title: r.title,
            path: r.path,
            year: r.year ?? null,
        ...
        pushAssistant({
            text:
                "Encontrei estas opções mais próximas. Você pode selecionar uma tabela, ou refinar:",
            suggestions,
    ...
    function onSelectTable(id) {
        ...
        // Mostra “tabela selecionada” e dá os 2 botões
        pushAssistant({
            text:
                `Tabela selecionada:\n` +
                `• ${doc.title}\n` +
            ...
            context: { selectedId: doc.id }, // <-- importante
        });
    }
    ...
    function handleFollowUpClick(label, context) {
        ...
        // se não é um followup “especial”, trata como mensagem normal
        onSend(label);
    }
                                        ...
                                        {msg.suggestions.map((s) => (
                                            <div key={s.id} className="suggestion-card">
                                                ...
                                                <div className="suggestion-meta">
                                                    ...
                                                    {s.source ? <span className="tag">Fonte: {s.source}</span> : null}
